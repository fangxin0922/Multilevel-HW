...
-   Level 1: time, measured in months
-   Level 2: individual students
-   $a_{ti}$: age in months at measurement point *t* for person *i*
-   $\pi_{0i}$: an individual student's "initial" vocabulary level, i.e., \# vocab words at $a_{ti}$ = 0
-   $\pi_{1i}$: an individual student's vocabulary growth rate per unit of time, i.e., expected \# vocab words growth per month
-   $\beta_{00}$: the "grand intercept" - mean initial vocab level for all students (when age is zero)
-   $\beta_{10}$: the "grand slope" - mean growth rate per month for all students
-   $\tau_{00}$: extent of individual variation around the mean initial vocab level (variance of $\pi_{0i}$)
-   $\tau_{11}$: extent of individual variation around the mean growth rate/curve (variance of $\pi_{1i}$)
-   $\rho=\frac{\tau_{10}}{\sqrt{\tau_{00}\tau_{11}}}$: correlation between initial status (intercepts $u_{0i}$) and rate of change (slopes $u_{1i}$)
-   $\epsilon_{ti}$: residual for measurement of vocab at time $t$ for individual $i$ - how far a particular measurement is different from the model's prediction
-   $\sigma^2$: variance of all of the residuals (how much of the data is left unexplained after our model?/how wrong is our model, on average (squared)?)
...
```{r}
m1 <- lmer(vocab ~ 1 + age12 + sex + (1 + age|pers), 
          data = dat)
```
...
```{r}
m2 <- lmer(vocab ~ 1 + age12*sex + (1 + age|pers), 
          data = dat)
```
